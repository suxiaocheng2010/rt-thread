...
static void hw_endpoint_init(uint8_t ep_addr, uint16_t wMaxPacketSize, uint8_t transfer_type)
{
  ...
  pico_trace_init("%s: ep%x, type: %x\n", __func__, ep_addr, transfer_type);
  ...
  *ep->buffer_control = 0;
  ...
  else
  {
    ...
    pico_trace_init("%s: %x%s ctrl: %x\n", __func__, num, dir==TUSB_DIR_IN?"in":"out", *(ep->endpoint_control));
  }
}
  ...
  if ( or_mask )
  {
    ...
    if ( or_mask & USB_BUF_CTRL_AVAIL )
    {
      if ( *ep->buffer_control & USB_BUF_CTRL_AVAIL )
      {
        ...
        pico_trace("Warn: ep %d %s was already available", tu_edpt_number(ep->ep_addr), ep_dir_string[tu_edpt_dir(ep->ep_addr)]);
      }
    ...
    for (uint8_t i = 0; remaining_buffers && i < USB_MAX_ENDPOINTS * 2; i++)
    {
		...
		if (tu_edpt_dir(ep->ep_addr) == TUSB_DIR_IN) {
			if (tu_edpt_number(ep->ep_addr) == 0) {
				...
				rt_usbd_ep_in_handler(&pico_udc, ep->ep_addr, ep->xferred_len);
                		hw_endpoint_reset_transfer(ep);
			}
		} else {
			if (tu_edpt_number(ep->ep_addr) == 0) {
				rt_usbd_ep0_out_handler(&pico_udc, ep->xferred_len);
                		hw_endpoint_reset_transfer(ep);
			} else {
				rt_usbd_ep_out_handler(&pico_udc, ep->ep_addr, ep->xferred_len);
                		hw_endpoint_reset_transfer(ep);
			}
		}
            }
...
static rt_err_t _ep_enable(struct uendpoint* ep)
{
	...
	hw_endpoint_init(ep->ep_desc->bEndpointAddress, ep->ep_desc->wMaxPacketSize, ep->ep_desc->bmAttributes);
	return RT_EOK;
}
